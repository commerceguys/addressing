...
/**
 * Generates JSON files for specific countries stored in resources/address_format and resources/subdivision.
 */
...
// Put countries you want to generate subdivision data and JSON from Google
$addCountries = ['PE'];
$write_directory = '../resources/';
...
foreach ($neededDirectories as $neededDirectory) {
    if (!is_dir($write_directory . $neededDirectory)) {
        mkdir($write_directory . $neededDirectory);
    }
}
...
file_put_contents('../resources/raw/url_list.txt', $urlList);

// Invoke aria2 and fetch the data.
echo "Downloading the raw data from Google's endpoint.\n";
exec('cd ../resources/raw && aria2c -u 16 -i url_list.txt');
...
$addressFormats = [];
$groupedSubdivisions = [];
foreach ($addCountries as $countryCode) {
    $definition = file_get_contents($write_directory . 'raw/' . $countryCode . '.json');
...
foreach ($addressFormats as $countryCode => $addressFormat) {
    file_put_json($write_directory . 'address_format/' . $countryCode . '.json', $addressFormat);
}
foreach ($groupedSubdivisions as $parentId => $subdivisions) {
    file_put_json($write_directory . 'subdivision/' . $parentId . '.json', $subdivisions);
}

// Generate the subdivision depths for each country.
$depths = generate_subdivision_depths($foundCountries);
file_put_json($write_directory . 'subdivision/depths.json', $depths);
